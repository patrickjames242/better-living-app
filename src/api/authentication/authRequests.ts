...
import {
  LogInSignUpJsonResponseObj,
  logInSignUpResponseObjValidator,
} from './validation';
...
(() => {
  ...
  const connectionStateObserver = async (
    state: AppState['realtimeUpdates']['connectionState'],
  ) => {
    if (
  ...
  addSelectedStateListener(
    state => state.realtimeUpdates.connectionState,
