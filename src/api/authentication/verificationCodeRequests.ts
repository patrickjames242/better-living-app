...
function _sendVerificationCode(
  ...
  return fetchFromAPI({
    path:
      ...
      (() => {
        switch (codeType) {
          case VerificationCodeType.signUp:
            return 'send-email-code';
          case VerificationCodeType.forgotMyPassword:
            return 'send-password-code';
        }
      })() +
      '/',
...
export function testVerificationCodeValidity(
  email: string,
  ...
  return fetchFromAPI<null>({
    ...
    path:
      basePath +
      `test-code-validity/?email=${email}&verification_code=${verificationCode}`,
  });
}
