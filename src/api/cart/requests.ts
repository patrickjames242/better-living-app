...
import {
  deleteCartEntryAction,
  incrementCartEntryPendingQuantityChangeAction,
  insertOrUpdateCartEntryAction,
} from '../../redux/cart';
import {
  CartMealEntryJsonResponseObj,
  CartProductEntryResponseObj,
} from './validation';
...
export function addProductToCart(productId: number) {
  return fetchFromAPI<CartProductEntryResponseObj>({
    ...
    store.dispatch(
      insertOrUpdateCartEntryAction(CartProductEntry.parse(result)),
    );
  });
}

export const changeProductEntryQuantity =
  batchQuantityUpdates<CartProductEntry>((entryId: string) => ({
    incrementReduxPendingQuantityChange: (incrementAmount: number) => {
      store.dispatch(
        incrementCartEntryPendingQuantityChangeAction(entryId, incrementAmount),
      );
    },
    ...
    quantityChangeTask: async (quantityChange: number) => {
      const url =
        baseProductEntriesUrl +
        entryId +
        '/update-quantity/?quantity_change=' +
        quantityChange;
...
export function updateMealEntryChoices(
  entryId: string,
  choices: MealEntryRequestChoice[],
) {
  return fetchFromAPI<CartMealEntryJsonResponseObj>({
...
export const changeMealEntryQuantity = batchQuantityUpdates<CartMealEntry>(
  (entryId: string) => ({
    incrementReduxPendingQuantityChange: (incrementAmount: number) => {
      store.dispatch(
        incrementCartEntryPendingQuantityChangeAction(entryId, incrementAmount),
...
function batchQuantityUpdates<Result>(
  getHandlers: (entryId: string) => BatchQuantityUpdatesHandlers<Result>,
): (entryId: string, changeType: IncrememntOrDecrement) => Promise<Result> {
  let entryCacheInfo = Map<
    string,
    {
      cachedPromiseInfo: {
  ...
  return function (productEntryId: string, changeType: IncrememntOrDecrement) {
    ...
    if (currentCacheInfo.taskHasStarted === false) {
      currentCacheInfo.timerId = setTimeout(() => {
        ...
        const getNewFetch: (
          totalPriorQuantityChanges?: number,
        ) => Promise<Result> = async totalPriorQuantityChanges => {
          const currentQuantityChange = currentCacheInfo.cachedQuantityChange;
          const totalAccumulatedQuantityChanges =
            (totalPriorQuantityChanges ?? 0) + currentQuantityChange;
          currentCacheInfo.cachedQuantityChange = 0;
          let response: Result;
          try {
            response = await handlers.quantityChangeTask(currentQuantityChange);
          } catch (error) {
            handlers.incrementReduxPendingQuantityChange(
              -totalAccumulatedQuantityChanges,
            );
            throw error;
          }
