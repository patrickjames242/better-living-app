...
import {
  CartMealEntryAPIResponseSchema,
  CartMealEntryJsonResponseObj,
  CartProductEntryAPIResponseSchema,
...
const InitialObjectsSchema = {
  ...
  required: ['product_entries', 'meal_entries'],
};
...
const initialObjectsValidator = new ajv({ allErrors: true }).compile(
  InitialObjectsSchema,
);
...
export function handleCartRealtimeUpdate(jsonData: any) {
  ...
  if (all_objects != undefined) {
    assertValidObjFromApi(
      initialObjectsValidator,
      'Cart Initial Objects',
      all_objects,
    );
    const entries = List<CartEntry>().withMutations(list => {
      all_objects.product_entries.forEach(x =>
        list.push(CartProductEntry.parse(x)),
      );
  ...
  const changed_object_type: ChangeModelType | undefined =
    jsonData.changed_object_type;
  ...
  switch (changed_object_type) {
    case ChangeModelType.product_entries:
      switch (change_type) {
        ...
        case ChangeUpdateType.update:
          store.dispatch(
            insertOrUpdateCartEntryAction(
              CartProductEntry.parse(changed_object),
            ),
          );
          break;
    ...
    case ChangeModelType.meal_entries:
      switch (change_type) {
        ...
        case ChangeUpdateType.update:
          store.dispatch(
            insertOrUpdateCartEntryAction(CartMealEntry.parse(changed_object)),
          );
          break;
        case ChangeUpdateType.delete:
          store.dispatch(deleteCartEntryAction(changed_object.id));
