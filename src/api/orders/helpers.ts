...
export function useCalculatedPriceInfoForOrder(order: Order): {
  ...
  const deliveryFee = (() => {
    if (order.userWantsOrderDelivered === false) {
      return null;
    }
  ...
  if (order.userPaidOnline) {
    ...
    subtotal = order.detailsJson
      .reduce<currency>((a1, a2) => {
        return a1.add(
          currency(a2.quantity).multiply(
            a2.entry_type === 'product' ? a2.product_price : a2.meal_price,
          ),
        );
      }, currency(0))
      .add(deliveryFee ?? 0);
  ...
  return {
    ...
    vat: vat.value,
    total: subtotal.add(vat).add(deliveryFee ?? 0).value,
    deliveryFee,
  };
}
