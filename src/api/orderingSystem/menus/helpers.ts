...
export function getMenuFromJsonResponseObj_orNull(
  apiResponseObj: MenuJsonResponseObj,
) {
  try {
    return new Menu(apiResponseObj);
  } catch (error) {
    console.error(
      'A menu could not be converted from json because of the following error ->',
      error,
      apiResponseObj,
    );
    return null;
...
function calculateIsCurrentMenu(menu: Menu) {
  ...
  const startTimeSeconds = getSecondsSinceDayBeginning(
    getMoment(menu.startTime),
  );
...
function calculateCurrentMenu(
  menusState: Map<number, Menu>,
): Optional<[Menu, Set<number>]> {
  ...
  while (currentNext.done !== true) {
    ...
    if (calculateIsCurrentMenu(menu) === true) {
      return [menu, calculateProductIdsSetForMenu(menu)];
    }
...
const currentMenuHolder: ValueBox<Optional<[Menu, Set<number>]>> = new ValueBox(
  calculateCurrentMenu(selectMenusState(store.getState())),
);
...
addSelectedStateListener(
  state => state.globalSettings.isOrderingSystemEnabled,
  () => {
    updateCurrentMenuIfNeeded();
  },
);
...
export const getCurrentMenu = () =>
  mapOptional(currentMenuHolder.value, ([menu]) => menu);

export function addCurrentMenuListener(
  listener: (menu: Optional<Menu>) => void,
) {
...
export const getProductIdsForCurrentMenu = () =>
  mapOptional(currentMenuHolder.value, ([_, ids]) => ids) ?? Set<number>();

export function addCurrentMenuProductIdsListener(
