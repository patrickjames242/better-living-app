...
} from '../../../redux/orderingSystem/productInfoTags';
import { getOrderingSystemObjRealtimeUpdater } from '../helpers';

export const handleProductInfoTagsRealtimeUpdate =
  getOrderingSystemObjRealtimeUpdater<ProductInfoTag>({
    jsonObjConverter: getProductInfoTagFromJsonResponseObj_orNull,
    allObjectsStateUpdater: allInfoTags =>
      store.dispatch(updateAllProductInfoTagsAction(allInfoTags)),
    insertOrUpdateStateUpdater: infoTag =>
      store.dispatch(insertOrUpdateProductInfoTagAction(infoTag)),
