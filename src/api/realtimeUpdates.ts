...
import {
  ...
  RealtimeUpdatesConnectionState,
  updateRealtimeUpdatesGotInitialUpdatesAction,
} from '../redux/realtimeUpdates';
...
function _tryConnectingWebsocketListener() {
  ...
  const getCurrentAuthToken = (state: AppState) =>
    state.authentication?.authToken ?? null;
  ...
  function onSuccessfulOpen(ws: WebSocket) {
    if (authTokenToUseInWebsocket === getCurrentAuthToken(store.getState())) {
      const removeReduxListener = addSelectedStateListener(
        getCurrentAuthToken,
        () => {
          removeReduxListener?.();
          ws.close();
  ...
  startWebsocketConnection(
    authTokenToUseInWebsocket,
    onCloseAfterSuccessfulOpen,
  )
    .then(onSuccessfulOpen)
    .catch(() => {
      startWebsocketConnection(
        authTokenToUseInWebsocket,
        onCloseAfterSuccessfulOpen,
        ...
        .catch(() => {
          ...
          unlisten = isInternetReachableNotification.addListener(
            isInternetReachable => {
...
function startWebsocketConnection(
  ...
  onCloseAfterSuccessfulOpen: () => void,
): Promise<WebSocket> {
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.onclose = function () {
      ...
      store.dispatch(
        updateRealtimeUpdatesConnectionStateAction(
          RealtimeUpdatesConnectionState.disconnected,
        ),
      );
    };
  });
