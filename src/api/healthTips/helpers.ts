...
export function getHealthTipFromObject_orThrow(object: {
  [index: string]: any;
}) {
  return new HealthTip(object as any);
}

export function getHealthTipFromObject_orNull(object: {
  [index: string]: any;
}) {
  try {
    ...
    console.error(
      'a health tip object could not be converted because of an error ->',
      error,
      object,
    );
    return null;
