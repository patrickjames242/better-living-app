...
const Spacer: React.FC<SpaceProps> = props => {
  return (
    <BaseSpacer
      renderSpacer={(lhs, rhs) => {
        const eitherSideIsASpace = [lhs.item, rhs.item].some(
          x => (x as React.ReactElement)?.type === Space,
        );
        if (eitherSideIsASpace) {
          return null;
        }
        return <Space {...props} />;
      }}
...
export interface BaseSpacerProps {
  renderSpacer: (
    leftItem: BaseSpacerChild,
    rightItem: BaseSpacerChild,
