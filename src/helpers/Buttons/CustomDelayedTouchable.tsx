...
const CustomDelayedTouchable: React.FC<
  ViewProps & CustomDelayedTouchableProps
> = props => {
  ...
  function onPressOut() {
    ...
    const timeTillButtonShouldTouchUp = Math.min(
      Math.max(
        (props.minDelayTillPressOut ?? 100) -
          (Date.now() - lastTouchDownDate.current),
        0,
      ),
      100,
    );

    currentTouchUpTimeout.current = setTimeout(
      action,
      timeTillButtonShouldTouchUp,
    );
  }
