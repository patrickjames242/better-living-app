...
import { ViewProps, StyleSheet, Animated } from 'react-native';
...
const HighlightButton = (() => {
  ...
  const HighlightButton: React.FC<HighlightButtonProps> = props => {
    ...
    const animateCoverView = useCallback(
      (show: boolean) => {
        ...
        {(() => {
          return (
            <Animated.View
              pointerEvents="none"
              style={[
                styles.highlightCoverView,
                {
                  opacity: coverViewOpacityAnimation,
                  backgroundColor: (
                    props.highlightColor ??
                    Color.gray(0).withAdjustedOpacity(0.05)
                  ).stringValue,
                },
              ]}
            />
