...
const LongTextAndIconButton = (() => {
  const styles = StyleSheet.create({
    ...
    iconHolder: {
      margin: 7,
      padding: 10,
      backgroundColor: new Color(255, 255, 255).withAdjustedOpacity(0.2)
        .stringValue,
      borderRadius: 10,
    },
    icon: {
  ...
  const LongTextAndIconButton = (props: LongTextAndIconButtonProps) => {
    ...
    const defaultBackgroundColor =
      props.backgroundColor ?? CustomColors.themeGreen;
    const actualBackgroundColor = isEnabled
      ? defaultBackgroundColor
      : defaultBackgroundColor.withAdjustedOpacity(
          0.5 * defaultBackgroundColor.opacity,
        );
      ...
      >
        <CustomizedText
          style={[
            styles.text,
            {
              textAlign: props.centerTitleText ?? false ? 'center' : undefined,
            },
          ]}
        >
          {props.text}
        ...
        <View style={styles.iconHolder}>
          <Image
            resizeMode="contain"
            style={[styles.icon, { opacity: isLoading ? 0 : 1 }]}
            source={props.iconSource}
          />
          <View
            style={[
              styles.activityIndicatorHolder,
