...
export const DEFAULT_NAV_SCREEN_OPTIONS: StackNavigationOptions = {
  headerShown: false,
  cardStyle: { backgroundColor: CustomColors.mainBackgroundColor.stringValue },
...
export function YUP_EDITING_FORM_PRICE_STRING(fieldName: string) {
  ...
  return yup
    ...
    .matches(
      /^(\$)?[0-9,]+(.[0-9]{2})?$/,
      `${fieldName} format is invalid. It must follow the format: '$15' or '$15.99'.`,
    );
}
...
export function YUP_PASSWORD_VALIDATOR(fieldName: string) {
  ...
  const capitalizedFieldName =
    fieldName.substr(0, 1).toUpperCase() + fieldName.substr(1);
  return yup
    .string()
    .min(
      minCharacters,
      `${capitalizedFieldName} must be at least ${minCharacters} characters long.`,
    )
    .required(`${capitalizedFieldName} is a required field.`);
}
...
export function lazilyImport<ModuleType, ImportedItem>(
  importCall: Promise<ModuleType>,
  elementGetter: (module: ModuleType) => ImportedItem,
): () => Optional<ImportedItem> {
...
export function compactMap<InputType, OutputType>(
  items: Array<InputType>,
  transformer: (input: InputType) => OutputType | undefined | null,
) {
...
export function computeNumberOfListColumns(props: {
  listWidth: number;
  maxItemWidth: number;
  sideInsets: number;
  ...
  return Math.ceil(
    (props.listWidth + props.horizontalItemSpacing - 2 * props.sideInsets) /
      (props.maxItemWidth + props.horizontalItemSpacing),
  );
}
...
export function getJsonValidatorErrorsText(
  ...
  return (
    ...
    '\n' +
    errors
      .map(x => `\tdataPath: '${x.dataPath}', message: ${x.message}`)
      .join(',\n') +
    '\n]'
  );
}
...
export function getPropsFromObject<ObjType extends object>(
  obj: ObjType,
  includedProps: (keyof ObjType)[],
) {
...
export function caseInsensitiveStringSort<ItemT>(
  ...args: ItemT extends string ? [] : [(item: ItemT) => string]
