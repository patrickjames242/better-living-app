...
export default class Parser<Result> {
  ...
  many(maxLength?: number): Parser<List<Result>> {
    return new Parser(input => {
      ...
      while (
        leftover.length >= 1 &&
        arrayToReturn.length < (maxLength ?? Number.MAX_VALUE)
      ) {
  ...
  then<NextResult>(
    nextParser: Parser<NextResult>,
    optional = false,
  ): Parser<[Result, Optional<NextResult>]> {
    return new Parser(input => {
      ...
      if (secondResult == null && optional === false) {
        return null;
      }
      return {
        result: [firstResult.result, secondResult?.result ?? null],
        leftover: secondResult?.leftover ?? firstResult.leftover,
      };
    });
  }
