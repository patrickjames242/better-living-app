...
export interface MultiColumnFlatListProps<ItemT>
  extends Omit<
    FlatListProps<ItemT>,
    'renderItem' | 'numColumns' | 'keyExtractor'
  > {
...
const MultiColumnFlatList = (() => {
  const MultiColumnFlatList = function MultiColumnFlatList<ItemT>(
    props: MultiColumnFlatListProps<ItemT>,
  ) {
    ...
    const listRowItemKeyExtractor = useCallback(
      (index: number) => {
    ...
    const listRowItemRenderer = useCallback(
      (itemIndex: number) => {
