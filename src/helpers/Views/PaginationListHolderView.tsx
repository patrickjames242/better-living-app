...
export type PaginationListChange<KeyT, ItemT> =
  | { changeType: PaginationListChangeType.delete; deletedItemId: KeyT }
  | { changeType: PaginationListChangeType.insertOrUpdate; changedItem: ItemT };

export interface PaginationListHolderViewRef<
...
export interface PaginationListHolderViewProps<
  KeyT extends string | number,
  ItemT,
> {
  batchSize: number;
...
const PaginationListHolderView = (() => {
  ...
  const PaginationListHolderView = <KeyT extends string | number, ItemT>(
    ...
    ref: Parameters<
      React.ForwardRefRenderFunction<
        PaginationListHolderViewRef<KeyT, ItemT>,
        PaginationListHolderViewProps<KeyT, ItemT>
      >
    >[1],
  ) => {
    const {
      batchSize,
      fetchMoreItems: propsFetchMoreItems,
      getItemId,
      getItemDate,
    } = props;

    const [items, setItems] = useState<OrderedMap<KeyT, ItemT>>(
      initialStateValues.items,
    );
    const [itemsAreBeingFetched, setItemsAreBeingFetched] = useState(
      initialStateValues.itemsAreBeingFetched,
    );

    const fetchEndedInError = useRef(initialStateValues.fetchEndedInError);
    const noMoreItemsAreAvailable = useRef(
      initialStateValues.noMoreItemsAreAvailable,
    );
    const lastMaxDate = useRef<Optional<moment.Moment>>(
      initialStateValues.lastMaxDate,
    );

    const resetState = useCallback(() => {
      batch(() => {
        fetchEndedInError.current = initialStateValues.fetchEndedInError;
        noMoreItemsAreAvailable.current =
          initialStateValues.noMoreItemsAreAvailable;
    ...
    const fetchMoreItems = useCallback(() => {
      if (itemsAreBeingFetched || noMoreItemsAreAvailable.current === true)
        return;

      fetchEndedInError.current = false;
      setItemsAreBeingFetched(true);

      propsFetchMoreItems(batchSize, lastMaxDate.current?.format() ?? undefined)
        .then(items => {
          if (items.length < batchSize) noMoreItemsAreAvailable.current = true;
          if (items.length >= 1)
            lastMaxDate.current = getItemDate(items[items.length - 1]);
          setItems(prevItems => {
            return (prevItems ?? OrderedMap<KeyT, ItemT>()).withMutations(
              map => {
    ...
    useImperativeHandle(
      ...
      () => {
        const shouldApplyChange = (
          change: PaginationListChange<KeyT, ItemT>,
        ) => {
        ...
        const applyChangeIfNeeded: PaginationListHolderViewRef<
          KeyT,
          ItemT
        >['applyChangeIfNeeded'] = change => {
    ...
    if (itemsAreBeingFetched && itemsArray.length <= 0) {
      return (
        <View style={styles.fullScreenLoaderHolder}>
          <CustomActivityIndicator size="large" />
        </View>
      );
    } else if (
      itemsAreBeingFetched === false &&
      fetchEndedInError.current &&
      (itemsArray?.length ?? 0) <= 0
    ) {
      return (
        <NoItemsToShowView
  ...
  return React.forwardRef(PaginationListHolderView) as <
    KeyT extends number | string,
    ItemT,
  >(
    props: PaginationListHolderViewProps<KeyT, ItemT> & {
      ref?: Parameters<
        React.ForwardRefRenderFunction<
          PaginationListHolderViewRef<KeyT, ItemT>,
          PaginationListHolderViewProps<KeyT, ItemT>
