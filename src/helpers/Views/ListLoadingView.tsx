...
const ListLoadingHolderView = (() => {
  ...
  const ListLoadingHolderView = (
    props: React.PropsWithChildren<ListLoadingHolderViewProps>,
  ) => {
    ...
    if (
      realtimeUpdatesState.connectionState ===
                  ...
                  if (hasFailedOnce.current === true) {
                    return (
                      <NoItemsToShowView
                        ...
                        subtitle="Could not connect to the server. We'll keep trying in the background!"
                      />
                    );
                  } else {
