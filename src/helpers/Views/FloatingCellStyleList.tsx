...
export interface FloatingCellStyleListProps<
  ItemType,
  SectionType extends SectionListData<ItemType>,
> extends SectionListProps<ItemType> {
...
const FloatingCellStyleList = (() => {
  const styles = StyleSheet.create({
    ...
    contentContainer: {
      ...LayoutConstants.maxWidthListContentContainerStyles(
        LayoutConstants.floatingCellStyles.maxWidth,
      ),
    },
  ...
  const FloatingCellStyleList = function <
    ItemType,
    SectionType extends SectionListData<ItemType>,
  >(props: FloatingCellStyleListProps<ItemType, SectionType>) {
    ...
    const SectionSeparatorComponent = useCallback(
      (args: any) => {
        ...
        const isBottomOfList =
          args.trailingItem == undefined && args.trailingSection == undefined;
        ...
        const space = (() => {
          if (
            isBottomOfList ||
            (isBottomOfHeader && (titleForSection?.(section) ?? null) == null)
          ) {
            return 0;
    ...
    return (
      <SectionList<ItemType>
        ...
        contentContainerStyle={[
          ...
          {
            paddingTop:
              props.titleForSection instanceof Function
                ? LayoutConstants.floatingCellStyles.sectionSpacing
                : undefined,
          },
...
const MealCreatorListViewSectionHeader = (() => {
  ...
  const FloatingCellStyleListSectionHeader = (
    props: FloatingCellStyleListSectionHeaderProps,
  ) => {
...
const FloatingCellStylesItemView = (() => {
  ...
  const FloatingCellStylesItemView: React.FC<
    FloatingCellStylesItemViewProps
  > = props => {
    return (
      <View
        style={[
          styles.root,
          {
            borderTopLeftRadius: props.isFirstInSection
              ? LayoutConstants.floatingCellStyles.borderRadius
              : 0,
            borderTopRightRadius: props.isFirstInSection
              ? LayoutConstants.floatingCellStyles.borderRadius
              : 0,
            borderBottomLeftRadius: props.isLastInSection
              ? LayoutConstants.floatingCellStyles.borderRadius
              : 0,
            borderBottomRightRadius: props.isLastInSection
              ? LayoutConstants.floatingCellStyles.borderRadius
              : 0,
          },
        ]}
