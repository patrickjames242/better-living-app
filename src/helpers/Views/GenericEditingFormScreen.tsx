      ...
      <CustomKeyboardAvoidingView style={styles.root}>
        ...
        >
          <TextFieldViewContainer
            errorMessage={
              props.formHasErrors ?? false
                ? 'This form has errors above.'
                : undefined
            }
          >
            <SpacerView
              style={styles.inputsHolder}
              space={GenericEditingFormScreenConstants.childrenSpacing}
            >
              {props.children}
            </SpacerView>
          </TextFieldViewContainer>
          ...
          <SpacerView
            space={10}
            children={
              props.longButtons?.map((x, index) => {
                return (
                  <LongTextAndIconButton
                    key={index}
                    {...x}
                    style={[styles.saveChangesButton, x.style]}
                  />
                );
              }) ?? []
            }
