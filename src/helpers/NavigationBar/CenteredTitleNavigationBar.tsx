...
const CenteredTitleNavigationBar = (() => {
  ...
  return function CenteredTitleNavigationBar(props: {
    ...
    const { onContentViewLayout, onTextHolderLayout } =
      useCenteringTextFunctionality(textRef);
...
function useCenteringTextFunctionality(textRef: React.RefObject<Text>) {
  ...
  function refreshTextMargins() {
    ...
    if (contentViewWidth == null || textHolderLayout == null) {
      return;
    }

    const contentViewCenter = contentViewWidth / 2;

    const distanceOfClosestEdgeToCenter = Math.min(
      contentViewCenter - textHolderLayout.x,
      textHolderLayout.width + textHolderLayout.x - contentViewCenter,
    );

    const newMargins = {
      marginLeft:
        contentViewCenter - distanceOfClosestEdgeToCenter - textHolderLayout.x,
      marginRight:
        textHolderLayout.x +
        textHolderLayout.width -
        contentViewCenter -
        distanceOfClosestEdgeToCenter,
    };
