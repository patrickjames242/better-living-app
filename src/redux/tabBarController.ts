...
} from '../UI/TabBarController/helpers';

export type ChangeTabBarPositionAction = CustomReduxAction<{
  newPosition: TabBarPosition;
}>;

export function changeTabBarPosition(
  newPosition: TabBarPosition,
): ChangeTabBarPositionAction {
  return {
...
export const getCurrentDefaultTabBarPosition = () =>
  calculateCurrentDesiredTabBarPosition();

function tabBarPositionReducer(
  state = getCurrentDefaultTabBarPosition(),
  action: ChangeTabBarPositionAction,
) {
  switch (action.type) {
