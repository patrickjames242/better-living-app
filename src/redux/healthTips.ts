...
export function insertHealthTipsAction(
  healthTips: HealthTip[],
...
export function updateHealthTipAction(
  healthTip: HealthTip,
...
export function deleteHealthTipAction(
  healthTipID: number,
...
export type HealthTipsActions =
  | InsertHealthTipsAction
  | UpdateHealthTipAction
  | DeleteHealthTipAction;

export function healthTipsReducer(
  state = Map<number, HealthTip>(),
  action: HealthTipsActions,
) {
  switch (action.type) {
    ...
    case ActionStrings.healthTips.UPDATE_HEALTH_TIP: {
      ...
      if (state.has(healthTip.id)) return state.set(healthTip.id, healthTip);
      else return state;
    }
