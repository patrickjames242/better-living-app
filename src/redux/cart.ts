...
export function updateEntireCartState(
  entries: List<CartEntry>,
): ReplaceCartEntriesAction {
  return {
...
export function insertOrUpdateCartEntryAction(
  entry: CartEntry,
...
export function incrementCartEntryPendingQuantityChangeAction(
  entryId: string,
  incrementAmount: number,
...
export function cartReducer(
  ...
  switch (action.type) {
    ...
    case ActionStrings.cart.INCREMENT_ENTRY_PENDING_QUANTITY_CHANGE: {
      const { entryId, incrementAmount } =
        action as IncrementCartEntryPendingQuantityChangeAction;
      ...
      return state.set(entryId, {
        ...oldEntry,
        pendingQuantityChangesInfo: (() => {
          const newQuantityChangeAmount =
            (oldEntry.pendingQuantityChangesInfo?.pendingChange ?? 0) +
            incrementAmount;
          return newQuantityChangeAmount === 0
            ? null
            : {
                originalQuantity:
                  oldEntry.pendingQuantityChangesInfo?.originalQuantity ??
                  oldEntry.entry.quantity,
