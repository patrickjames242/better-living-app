...
export function updateGlobalSettingsAction(globalSettings: GlobalSettings): UpdateGlobalSettingsAction{
    return {
...
export function globalSettingsReducer(state = defaultGlobalSettings, action: GlobalSettingsActions): GlobalSettings{
    switch (action.type){
...
import('./store').then(module => {
    ...
    AsyncStorage.getItem(globalSettingsKey).then(jsonString => {
        ...
        try{
            jsonString != null && module.default.getState().globalSettings == null &&
            module.default.dispatch(updateGlobalSettingsAction(new GlobalSettings(JSON.parse(jsonString))));
        } catch (error) {
            console.log(error);
        }
    });

    module.addSelectedStateListener(state => state.globalSettings, globalSettings => {
