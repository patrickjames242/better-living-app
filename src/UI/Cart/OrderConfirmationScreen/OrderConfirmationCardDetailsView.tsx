...
function formatCreditCardNumber(string: string): string {
  ...
  return (
    (cardNumberGroupParser.many(4).parse(string)?.result ?? []).join(' ') ?? ''
  );
}
...
function formatCVVCode(string: string) {
  const parser = CustomParsers.digit
    .ignoreNulls()
    .many(4)
    .map(x => x.join(''));
...
const OrderConfirmationCardDetailsView = (() => {
  ...
  const OrderConfirmationCardDetailsView = (
    props: OrderConfirmationCardDetailsViewProps,
  ) => {
    ...
    const opacity = useRef(
      new Animated.Value(currentDesiredOpacityValue),
    ).current;
...
interface NumberTextFieldViewProps<
  Key extends keyof OrderConfirmationScreenValues,
> extends FormikTextFieldViewProps<Key> {
  textFormatter: (text: string) => string;
}

function NumberTextFieldView<Key extends keyof OrderConfirmationScreenValues>(
  props: NumberTextFieldViewProps<Key>,
) {
  const [textFieldCursorPosition, setTextFieldCursorPosition] =
    useState<Optional<{ position: number }>>(null);
  ...
  useEffect(() => {
    if (
      ...
      (cardTextFieldRef.current as any)?.setSelectionRange(
        textFieldCursorPosition.position,
        textFieldCursorPosition.position,
      );
    }
    ...
    <TextFieldView
      ...
      onChangeText={value => {
        ...
        batch(() => {
          if (Platform.OS === 'web') {
            const newPosition = (() => {
              const currentCursorPosition = (cardTextFieldRef.current as any)
                ?.selectionEnd as number;
              if (value.length > currentTextValue.current.length) {
                const characterTypedPosition = Math.max(
                  currentCursorPosition - 1,
                );
                const characterTyped = value[characterTypedPosition];
                const newCharacterPosition = newValue.indexOf(
                  characterTyped,
                  characterTypedPosition,
                );
                return newCharacterPosition === -1
                  ? newValue.length
                  : newCharacterPosition + 1;
              } else {
