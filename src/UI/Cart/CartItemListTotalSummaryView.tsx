...
const CartTotalSummaryView = (() => {
  ...
  const styles = StyleSheet.create({
    ...
    finalTotalHolderBox: {
      ...
      backgroundColor:
        CustomColors.mainBackgroundColor.withAdjustedOpacity(0.8).stringValue,
      borderRadius: 8,
    },
    finalTotalText: {
  ...
  const CartTotalSummaryView = (props: CartTotalSummaryViewProps) => {
    ...
    const subtotal = useMemo(() => {
        ...
        const currentValuePrice = (() => {
          const quantity = (() => {
            if (cv.pendingQuantityChangesInfo) {
              return currency(
                cv.pendingQuantityChangesInfo.originalQuantity,
              ).add(cv.pendingQuantityChangesInfo.pendingChange);
            } else {
      ...
      deliveryFee,
      props.entries,
      props.includeDeliveryFee,
    ]);

    const vatPercentage = useSelector(
      state => state.globalSettings.vatPercentage,
    );
    ...
    return (
      <SpacerView style={styles.root} space={10}>
        <SpacerView style={styles.subtotalItemsBox} space={8}>
          {props.includeDeliveryFee && deliveryFee > 0 && (
            <View style={styles.amountItemBox}>
              <CustomizedText style={styles.subtotalItemText}>
                Delivery Fee:
              </CustomizedText>
              <CustomizedText style={styles.subtotalItemText}>
                {currency(deliveryFee).format()}
              </CustomizedText>
            </View>
          )}
          <View style={styles.amountItemBox}>
        ...
        <View style={[styles.amountItemBox, styles.finalTotalHolderBox]}>
          ...
          <CustomizedText style={styles.finalPriceText}>
            {finalTotal.format()}
          </CustomizedText>
        </View>
      </SpacerView>
