...
const CartItemListScreen = (() => {
  ...
  const CartItemListScreen = (
    props: StackScreenProps<CartNavStackParamList, 'CartItemList'>,
  ) => {
    useTabBarControllerChildRootScreenPopToTopFunctionality(
    ...
    const currentlyOpenDrawerID = useRef(
      new ValueBox<Optional<string>>(null),
    ).current;
    const bottomGradientViewRef =
    ...
    const listView = useMemo(() => {
      return (
        <FloatingCellStyleList<CartEntriesMapValue, Section>
          contentContainerStyle={[
            styles.listViewContent,
            {
              paddingBottom:
                bottomButtonHolderHeight +
          ...
          sections={sections}
          onScroll={event => {
            bottomGradientViewRef.current?.gradientHolder?.notifyThatScrollViewScrolled(
              event,
            );
          ...
          renderItem={({ item }) => {
            return (
              <CartItemListItemView
                entry={item}
                currentlyOpenDrawerID={currentlyOpenDrawerID}
              />
            );
          }}
        />
          ...
          {(() => {
            if (sortedCartEntries.length <= 0) {
              return (
                <NoItemsToShowView
                  title="Empty Cart"
                  subtitle="Looks like you havn't made any choices yet."
                  imageSource={require('./shopping-cart.png')}
                  imageStyle={styles.emptyCartImage}
                />
              );
            } else {
              return (
