...
import {
  TabBarPosition,
  useSetUpWindowDimensionsObserver,
  calculateCurrentDesiredTabBarPosition,
  useWindowDimensionsNotificationListener,
...
import {
  customerSelections,
  employeeAndManagerSelections,
  getDefaultTabBarSelectionForUserObject,
...
const TabBarController = (() => {
  ...
  function useCurrentInitialRouteName() {
    ...
    return useMemo(() => {
      switch (appState.initialAppScreen?.type) {
        ...
        default: {
          const currentUser =
            store.getState().authentication?.userObject ?? null;
  ...
  function _CustomTabBarContentView() {
    ...
    return (
      <Tab.Navigator initialRouteName={initialRouteName}>
        <Tab.Screen
          name={TabBarSelection.todaysOrders}
  ...
  return function TabBarController(
    props: StackScreenProps<RootNavigationViewParams, 'MainInterface'>,
  ) {
    ...
    const [currentTabBarSelection, setCurrentTabBarSelection] = useState(
      useCurrentInitialRouteName(),
    );
    ...
    useEffect(() => {
      return props.navigation.addListener('state', event => {
        const tabBarState = event.data.state.routes.find(
          x => x.name === mainInterfaceKey,
        )?.state;
        if (tabBarState && 'routeNames' in tabBarState) {
          const newSelection = (tabBarState.routeNames as string[])[
            tabBarState.index!
          ] as TabBarSelection;
    ...
    const changeTab = useCallback(
      (selection: TabBarSelection) => {
        ...
        if (
          store.getState().authentication == null &&
          tabBarSelectionInfo.requiresAuthentication
        ) {
    ...
    useEffect(() => {
      return addSelectedStateListener(
        ...
        authentication => {
          ...
          if (
            selectionsForCurrentUser.includes(currentTabBarSelection) ===
              false ||
            (authentication == null &&
              getInfoForTabBarSelection(currentTabBarSelection)
                .requiresAuthentication)
          ) {
            changeTab(
              getDefaultTabBarSelectionForUserObject(
                authentication?.userObject ?? null,
              ),
            );
          }
        },
    ...
    const updateTabBarPositionIfNeeded = useCallback(
    ...
    useWindowDimensionsNotificationListener(
      dimensions => {
        const newPosition = calculateCurrentDesiredTabBarPosition(
          dimensions.width,
        );
    ...
    return (
      <TabBarControllerContext.Provider value={tabBarControllerContextValue}>
        <View
            ...
            if (currentTabBarState.tabBarPosition === TabBarPosition.side) {
              return (
                <View style={styles.sideBarHolder}>
                  <SideTabBar
                    selectedTab={currentTabBarSelection}
