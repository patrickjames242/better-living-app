    ...
    >
      <View style={tabBarStyles.contentView}>
        {currentTabBarSelections.map((selection, index) => {
          ...
          const imageTintColor = isSelected
            ? CustomColors.themeGreen
            : Color.gray(0.75);
          ...
          return (
            <BouncyButton
              key={index}
              style={tabBarStyles.tabBarButton}
              onPress={onPress}
              bounceScaleValue={1.3}
            >
              {(() => {
                switch (Platform.OS) {
                  case 'web':
                  ...
                  default:
                    return (
                      <Image
                        resizeMode="contain"
                        source={info.icons.png}
                        style={[
                          tabBarStyles.tabBarButtonImage,
                          { tintColor: imageTintColor.stringValue },
                        ]}
                      />
                    );
                }
