...
const CustomTabNavigator = (() => {
    ...
    const CustomTabNavigator = function (props: DefaultNavigatorOptions<{}>) {
        ...
        const [currentScreenIndices, setCurrentScreenIndices] = useState<List<number>>(List([state.index]));
        const [animationIsInProgress, setAnimationIsInProgress] = useState(false);

        interface AnimatedValues {
            [screenIndex: number]: { translateX: Animated.Value, opacity: Animated.Value }
        }
        ...
        useUpdateEffect(() => {
            ...
            const oldScreenIndex = currentScreenIndices.get(currentScreenIndices.size - 1)!;
            ...
            const oldViewTranslateX = animatedValues.current[oldScreenIndex].translateX;
            const newViewTranslateX = animatedValues.current[newScreenIndex].translateX;
        ...
        return <View style={[styles.root, {
            ...
            {(() => {
                return currentScreenIndices.map(index => {
                    ...
                    return <Animated.View key={index} style={[styles.childContainer, {
