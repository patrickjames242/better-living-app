...
export const windowDimensionsDidChangeNotification =
  Notification<WindowDimensions>();

export function useWindowDimensionsNotificationListener(
  listener: (dimensions: WindowDimensions) => void,
...
export function calculateCurrentDesiredTabBarPosition(
  screenWidth?: number,
): TabBarPosition {
...
export const TabBarControllerContext =
  React.createContext<Optional<TabBarControllerContextValue>>(null);

export function useTabBarControllerContext() {
  const context = useContext(TabBarControllerContext);
  if (context == null) {
    throw new Error(
      'you cannot call useTabBarControllerNavigation outside of a TabBarControllerContext provider',
    );
  }
  return context;
}
...
export const shouldPopTabBarControllerChildToTop =
  Notification<TabBarSelection>();

// this is placed on the root screen to dismiss to the root screen when button is pressed
export function useTabBarControllerChildRootScreenPopToTopFunctionality<
  ScreenParams extends Record<string | number | symbol, any>,
  Screen extends keyof ScreenParams,
>(
  selection: TabBarSelection,
  screenProps: StackScreenProps<ScreenParams, Screen>,
) {
  useEffect(() => {
