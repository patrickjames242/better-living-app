...
import CreateOrEditTipEditButton, {
  CreateOrEditTipEditButton_EditButtonType,
} from '../CreateOrEditTipEditButton';
...
const YoutubeVideoView = (() => {
  ...
  const YoutubeVideoView = (props: YoutubeVideoViewProps) => {
    ...
    const [fetchError, setFetchError] =
      useState<Optional<FetchErrorType>>(null);

    useEffect(() => {
      fetchYoutubeVideo(props.videoId)
        .then(video => {
          if (video == null) {
    ...
    return (
      <CreateOrEditTipAttachmentContainer style={styles.root}>
        <Spacer space={10}>
          <AspectRatioView
            heightPercentageOfWidth={9 / 16}
            style={styles.imageHolder}
            innerHolderViewStyle={styles.innerImageHolder}
          >
            {videoInfo != null && (
              <Image
                style={styles.image}
                source={{ uri: videoInfo.thumbnailImageURL }}
              />
            )}
            {fetchError != null && (
              <Image
                style={styles.errorIcon}
                source={AssetImages.warningIcon}
              />
            )}
          </AspectRatioView>
          ...
          <CreateOrEditTipEditButton
            onPress={props.onDeleteButtonPressed}
            buttonType={CreateOrEditTipEditButton_EditButtonType.delete}
          />
        </Spacer>
