...
const CreateOrEditTipAudioFilesSection = (() => {
    ...
    const CreateOrEditTipAudioFilesSection = (props: CreateOrEditTipAudioFilesSectionProps) => {

        const existingAudioFiles = props.audioFiles.toArray().filter(x => props.deletedAudioFileIds.includes(x.id) === false);
        const addedAudioFiles = props.addedAudioFiles.toArray();

        return <TextFieldViewContainer topTitleText="Audio Files">
            ...
            <Spacer space={15} children={[
                ...(existingAudioFiles.map((x, i) => {
                    return <AudioFileView
                        ...
                        onDeleteButtonPressed={() => props.onUserWantsToRemoveExistingFile(x.id)}
                    />
                })),
                ...
                <AddNewFileButton key="add-new-file-button" onUserWantsToAddFile={props.onUserWantsToAddFile} />,
            ]} />
