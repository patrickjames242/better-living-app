...
const TipsDetailYTVideoView = (() => {
  ...
  const TipsDetailYTVideoView = (props: TipsDetailYTVideoViewProps) => {
    return (
      <AspectRatioView style={styles.root} heightPercentageOfWidth={9 / 16}>
        {Platform.select({
          web: (
            <iframe
              style={{ width: '100%', height: '100%' }}
              src={`https://www.youtube.com/embed/${props.ytVideoID}`}
              frameBorder="0"
          ...
          default: (
            <WebView
              source={{
                uri: `https://www.youtube.com/embed/${props.ytVideoID}`,
              }}
              javaScriptEnabled
              domStorageEnabled
              onLoadEnd={() => {
