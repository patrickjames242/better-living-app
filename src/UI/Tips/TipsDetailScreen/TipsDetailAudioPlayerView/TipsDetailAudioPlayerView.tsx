...
} from './TipsDetailAudioPlayerViewPlayPauseButton';
import TipsDetailAudioPlayer, {
  TipsDetailAudioPlayerPlayableState,
} from './TipsDetailAudioPlayer';
  ...
  const TipsDetailAudioPlayerView = (props: TipsDetailAudioPlayerViewProps) => {
    ...
    return (
      <SpacerView
        style={styles.root}
        space={
          playPauseState === PlayPauseButtonState.reload
            ? 10
            : LayoutConstants.floatingCellStyles.padding
        }
      >
        <PlayPauseButton
        ...
        <SpacerView style={styles.scrubberHolder} space={10}>
          {(() => {
            if (playPauseState === PlayPauseButtonState.reload) {
              return (
                <SpacerView style={styles.errorView} space={10}>
                  <Image
                  ...
                  >{`An error occured when trying to load this audio on your device. Try reloading or using the website version of this app.`}</CustomizedText>
                </SpacerView>
              );
            } else {
              return (
