...
const LogInSignUpScreenTemplate = (() => {
  ...
  const LogInSignUpScreenTemplate = (props: LogInSignUpScreenTemplateProps) => {
    const navigation =
      useNavigation<
        CompositeNavigationProp<
          NavigationProp<LogInSignUpUIParams>,
          NavigationProp<RootNavigationViewParams>
        >
      >();

    const bottomScreenButtonRef =
        ...
        <CustomKeyboardAvoidingView style={styles.innerRootView}>
          <View style={{ flex: 1 }}>
                ...
                iconSource={(() => {
                  switch (props.topLeftButtonType) {
                    case ExitOrBackButton.back:
                      return AssetImages.backArrowIcon;
                    case ExitOrBackButton.exit:
                      return AssetImages.xIcon.green;
                  }
                })()}
                iconSize={17}
                onPress={() => {
            ...
            <ScrollView
              ...
              onScroll={x =>
                bottomScreenButtonRef.current?.gradientHolder?.notifyThatScrollViewScrolled(
                  x,
                )
              }
