...
const OrderItemsView = (() => {
      ...
      <View style={styles.segmentView}>
        ...
        <View style={styles.segmentCenter}>
          ...
          {props.item.quantity >= 2 && (
            <CustomizedText style={styles.segmentSubtitleLabel}>
          ...
          {props.item.entry_type === 'meal' && props.item.choices.length >= 1 && (
            <>
              ...
              <Spacer
                space={15}
                children={props.item.choices.map((x, index) => {
                  return (
                    <View key={index} style={styles.segmentMealChoiceView}>
                      <CustomizedText
                        style={styles.segmentMealChoiceCategoryText}
                      >
                        {x.category_name + ':'}
                      </CustomizedText>
                      <CustomizedText
                        style={styles.segmentMealChoiceProductTitle}
                      >
                        {x.chosen_product_name}
                      </CustomizedText>
                    </View>
                  );
                })}
        ...
        <CustomizedText style={styles.segmentPriceLabel}>
          {currency(individualPrice * props.item.quantity).format()}
        </CustomizedText>
      </View>
    );
  };
  ...
  const OrderItemsView = (props: OrderItemsViewProps) => {
    return (
      <OrderDetailTitleContainer title="Order Items">
        {/* eslint-disable-next-line react/no-children-prop */}
        <Spacer
          space={17}
          children={props.orderDetailsJson.map((x, index) => (
            <SegmentView key={index} item={x} />
          ))}
        />
