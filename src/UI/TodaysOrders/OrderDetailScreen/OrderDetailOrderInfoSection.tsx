...
const OrderInfoSection = (() => {
  ...
  const SegmentView = (
    ...
    return (
      <View style={styles.segmentView}>
        <CustomizedText style={styles.segmentTitle}>
          {props.title}
        </CustomizedText>
        {(() => {
          if (props.value == null) return props.children;
            ...
            if (props.imageSource != null) {
              return (
                <View style={styles.segmentValueWithImageHolder}>
                  <Image
                    style={styles.segmentValueImage}
                    source={props.imageSource}
                  />
                  <Space space={7} />
                  <CustomizedText style={styles.segmentValueText}>
                    {props.value}
                  </CustomizedText>
                </View>
              );
  ...
  const OrderInfoSection = (props: OrderInfoSectionProps) => {
    ...
    const toggleCompleted = useCallback(() => {
      ...
      updateOrderIsCompleted(order.id, !order.isCompleted)
        .then(order => {
          onOrderUpdate(order);
        })
        .catch(error => {
    ...
    const isUserEmployeeOrManager = useSelector(state => {
      const currentUserType =
        state.authentication?.userObject.userType ?? UserType.customer;
      return [UserType.employee, UserType.manager].includes(currentUserType);
    });

    return (
      <OrderDetailTitleContainer title="Order Info">
            ...
            <SegmentView
              ...
              imageSource={
                props.order.userPaidOnline
                  ? require('../../Cart/OrderConfirmationScreen/icons/pay.png')
                  : require('../../Cart/OrderConfirmationScreen/icons/buy.png')
              }
              value={
                props.order.userPaidOnline ? 'Paid Online' : 'Payment In Person'
              }
            />
            <SegmentView
              title="Completion Status"
              value={props.order.isCompleted ? 'Completed' : 'Incomplete'}
            />
          </SpacerView>
          <Space space={10} />
          <SpacerView space={15} style={{ flex: 1 }}>
            <SegmentView title="Order Date">
              <CustomizedText style={styles.segmentValueText}>
                {props.order.creationDate.format('LLL')}
              </CustomizedText>
            </SegmentView>
            <SegmentView
              title="Pick Up Or Delivery"
              imageSource={
                props.order.userWantsOrderDelivered
                  ? require('../../Cart/OrderConfirmationScreen/icons/motorcycle.png')
                  : require('../../Cart/OrderConfirmationScreen/icons/order.png')
              }
              value={
                props.order.userWantsOrderDelivered ? 'Delivery' : 'Pick Up'
              }
            />
            <SegmentView
              title="Total Price"
              value={currency(totalPrice).format()}
            />
          </SpacerView>
        </View>
        {isUserEmployeeOrManager && (
          <RoundedTextBouncyButton
            isEnabled={completedIsLoading === false}
            text={
              completedIsLoading
                ? 'Loading...'
            ...
            style={styles.setCompletedButtonStyle}
