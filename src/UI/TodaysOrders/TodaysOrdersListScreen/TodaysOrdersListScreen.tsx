...
const TodaysOrdersListScreen = (() => {
  ...
  const TodaysOrdersListScreen = (
    props: StackScreenProps<TodaysOrdersNavStackParams, 'TodaysOrdersList'>,
  ) => {
    ...
    const orderSections: SectionType[] = useMemo(() => {
      ...
      ordersReduxState
        ...
        .forEach(value => {
          const orderWasCreatedToday = value.creationDate
            .clone()
            .startOf('day')
            .isSame(startOfToday());
    ...
    const respondToButtonPressed = useCallback(
      (order: Order) => {
          ...
          {(() => {
            if (orderSections.length < 1) {
              return (
                <NoItemsToShowView
                  title="No Orders"
                  subtitle="No orders have been placed as yet for the day."
                />
              );
            }
            return (
              <MultiColumnSectionList<Order, SectionType>
                ...
                SectionSeparatorComponent={leadingTrailingInfo => {
                  const size = (() => {
                    if (leadingTrailingInfo.trailingItem !== undefined) {
                      ...
                      leadingTrailingInfo.trailingSection !== undefined
                    ) {
                      return 30;
                    } else {
                      return 0;
                ...
                renderSectionHeader={info => {
                  return (
                    <CustomizedText style={styles.sectionHeaderText}>
                      {(info.section.realSection as SectionType).title}
                    </CustomizedText>
                  );
                }}
                renderItem={item => {
                  return (
                    <TodaysOrdersListItemView
                      order={item}
                      onPress={respondToButtonPressed}
                    />
                  );
                }}
              />
