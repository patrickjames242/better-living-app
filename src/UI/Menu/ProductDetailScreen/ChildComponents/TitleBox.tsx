...
const TitleBox = (() => {
  ...
  return function TitleBox(props: { product: Product }) {
    const allInfoTagsMap = useSelector(
    ...
    const categoryTitles = useMemo(() => {
      return (
        currentMenu?.categories
          .filter(x => x.productIds.contains(props.product.id))
          .map(x => x.title)
          .sort(caseInsensitiveStringSort()) ?? List<string>()
      );
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [currentMenu]);

    const infoTags = useMemo(() => {
      return compactMap(props.product.infoTagIds.toArray(), x =>
        allInfoTagsMap.get(x),
    ...
    return (
      <SpacerView style={styles.root} space={10}>
        ...
        {categoryTitles.size >= 1 && (
          ...
          >
            <Image
              ...
              source={require('./forkAndKnife.png')}
            />
            <CustomizedText style={styles.categoryText}>
              {categoryTitles.join(' • ')}
            </CustomizedText>
          </SpacerView>
        )}
        {infoTags.length >= 1 && (
          <SpacerView
            space={foodTagViewsSpacing}
            style={styles.foodTagViewsHolder}
          >
            {infoTags.map(x => (
              <FoodTagView key={x.id} title={x.title} />
            ))}
          </SpacerView>
        )}
      </SpacerView>
