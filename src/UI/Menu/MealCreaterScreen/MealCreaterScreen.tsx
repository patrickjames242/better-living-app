...
import {
  addMealEntryToCart,
  MealEntryRequestChoice,
  updateMealEntryChoices,
...
const MealCreatorScreen = (() => {
          ...
          category &&
            map.set(category.id, {
              ...
              productIds: category.productIds.filter(
                x => currentMenuProductIds.has(x) && allReduxProducts.has(x),
              ),
            });
        });
      });
  ...
  const MealCreatorScreen = (
    props: StackScreenProps<MenuNavStackParams, 'MealCreator'>,
  ) => {
    const mealId = (() => {
    ...
    const { listViewSections, currentMealCategoriesMap } =
      useListViewSections(meal);
      ...
      return Map<number, Optional<number>>().withMutations(map => {
        if ('mealIdToCreateEntryFor' in props.route.params) {
          ...
          props.route.params.mealEntryToEdit.choices.forEach(x => {
            if (
              currentMealCategoriesMap
                .get(x.mealProductCategoryId)
                ?.productIds.has(x.chosenProductId) ??
              false
            ) {
        ...
        const newState = Map<number, Optional<number>>().withMutations(map => {
          currentMealCategoriesMap.forEach((category, categoryId) => {
            const chosenProductId =
              mapOptional(oldState.get(categoryId), x => {
      ...
      const areSelectionsValid =
        chosenProductIds.some((productId, categoryId) => {
          return (
            ...
            (currentMealCategoriesMap
              .get(categoryId)
              ?.productIds.has(productId) ?? false) === false
          );
        }) === false && currentMealCategoriesMap.size === chosenProductIds.size;
    ...
    function onSubmitButtonPressed() {
      ...
      const promise = (() => {
        if ('mealEntryToEdit' in props.route.params) {
          return updateMealEntryChoices(
            props.route.params.mealEntryToEdit.id,
            choices,
          );
        } else {
    ...
    const initialNumToRender = useMemo(() => {
      return Math.ceil(
        Dimensions.get('window').height /
          MealCreatorConstants.foodSections.rowHeight,
      );
    }, []);

    const bottomButtonHolderRef =
      useRef<BottomScreenButtonWithGradientRef>(null);

    const updateSelectedProductIdForCategory = useCallback(
    ...
    const sectionList = useMemo(() => {
      return (
        <FloatingCellStyleList<Product, MealCreatorSection>
          style={styles.sectionList}
          contentContainerStyle={{
            paddingBottom:
              bottomButtonViewHeight + bottomButtonTopAndBottomInsets,
          }}
          sections={listViewSections}
          titleForSection={section => section.title}
          onScroll={event =>
            bottomButtonHolderRef.current?.gradientHolder?.notifyThatScrollViewScrolled(
              event,
            )
          }
          ...
          renderItem={({ item, section }) => {
            ...
            return (
              <MealCreatorListViewItem
                ...
                isSelected={
                  chosenProductIds.get(_section.categoryId) === item.id
                }
              />
            );
          }}
              ...
              <>
                ...
                <BottomScreenButtonWithGradient
                  ...
                  buttonProps={{
                    ...(() => {
                      if (
                        MealCreatorPropKeys.mealEntryToEdit in
                        props.route.params
                      ) {
