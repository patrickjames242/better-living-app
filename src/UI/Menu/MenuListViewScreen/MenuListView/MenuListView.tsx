  ...
  return function MenuListView(props: MenuListViewProps) {
    ...
    const listViewSections = useMemo(() => {
      const categoriesToDisplay =
        selectedCategory === ALL_CATEGORIES_CATEGORY
          ? sortedCategories
          : List([selectedCategory]);
      return (
        categoriesToDisplay.toArray().map(category => {
          return {
        ...
        <View style={[styles.root]}>
          <MultiColumnSectionList<number, { data: number[] }>
            ...
            ItemSeparatorComponent={() => {
              return (
                <View style={{ height: itemSpacing, width: itemSpacing }} />
              );
            }}
            ...
            renderSectionHeader={info => {
              return (
                <CustomizedText style={styles.sectionHeaderText}>
                  {
                    (info.section.realSection.menuCategory as MenuCategory)
                      .title
                  }
                </CustomizedText>
              );
