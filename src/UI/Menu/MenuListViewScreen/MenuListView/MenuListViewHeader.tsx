...
const MenuListViewHeader = (() => {
  ...
  return function MenuListViewHeader(props: MenuListViewHeader) {
    const firstName = useSelector(
      state => state.authentication?.userObject.firstName,
    );
    return (
      <View style={styles.root}>
        <View style={styles.topTitlesHolder}>
          <CustomizedText style={styles.topTitles_topBoldTitle}>{`Hello, ${
            firstName ?? 'Guest'
          }`}</CustomizedText>
          <CustomizedText style={styles.topTitles_subtitle}>
...
const MenuCategoriesListView = (() => {
  ...
  return function MenuCategoriesListView() {
    ...
    const allCategories = useMemo(() => {
      return listViewContext.allSortedCategories
        .unshift(ALL_CATEGORIES_CATEGORY)
        .toArray();
    ...
    return (
      <View style={styles.root}>
        <FlatList
          ...
          renderItem={({ item }) => {
            return (
              <MenuCategoriesListViewItem
                text={item.title}
                onPress={() => listViewContext.setSelectedCategory(item)}
                isSelected={
                  item.title === listViewContext.selectedCategory.title
                }
              />
            );
          }}
...
const MenuCategoriesListViewItem = (() => {
  ...
  return function MenuCategoriesListViewItem(
    props: MenuCategoriesListViewItemProps,
  ) {
    return (
      <BouncyButton
          ...
          style={[
            styles.text,
            {
              color: props.isSelected
                ? 'white'
                : CustomColors.offBlackTitle.stringValue,
            },
          ]}
        >
          {props.text}
