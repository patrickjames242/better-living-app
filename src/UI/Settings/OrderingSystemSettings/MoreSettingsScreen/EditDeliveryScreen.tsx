...
const EditDeliveryFeeScreen = (
  props: StackScreenProps<SettingsNavStackParams, 'EditDeliveryFee'>,
) => {
  const intialValues: Values = useMemo(() => {
    return {
      deliveryFee: currency(
        store.getState().globalSettings.deliveryFee,
      ).format(),
    };
  }, []);
  ...
  return (
    <Formik
      ...
      validationSchema={yup.object({
        deliveryFee: YUP_EDITING_FORM_PRICE_STRING('Vat Percentage').required(
          'Vat Percentage field is required',
        ),
      })}
      onSubmit={(values, { setSubmitting }) => {
        const deliveryFee = currency(values.deliveryFee.trim()).value;
        updateGlobalSettings({ delivery_fee: deliveryFee })
          .finally(() => {
            setSubmitting(false);
          })
          .then(() => {
            ...
            longButtons={[
              {
                ...
                isEnabled:
                  formik.isValid &&
                  formik.dirty &&
                  currency(formik.initialValues.deliveryFee).value !==
                    currency(formik.values.deliveryFee).value,
              },
            ]}
          >
            <FormikTextFieldView<Values>
              formikFieldName="deliveryFee"
              topTitleText="Delivery Fee"
