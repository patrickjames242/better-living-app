...
import {
  ...
  updateMeal,
  deleteMeal,
} from '../../../../../api/orderingSystem/meals/requests';
...
const MealEditOrCreationScreen = (() => {
  function submitForm(
    ...
    values: MealEditOrCreationValues,
  ) {
    const requestObj: MealRequestObj = {
      title: values.title,
      price: currency(values.priceString.trim()).value,
      product_categories: values.productCategoryIds
        .toArray()
        .map(id => ({ id, order_num: 0 })),
    };
    return mealId == null
      ? createNewMeal(requestObj)
      : updateMeal(mealId, requestObj);
  }
  ...
  const MealEditOrCreationScreen = (
    ...
    const initialValues: MealEditOrCreationValues = useMemo(() => {
      const productCategoriesReduxState =
        store.getState().orderingSystem.mealCategories;
      return {
        title: meal?.title ?? '',
        ...
        productCategoryIds: Set<number>().withMutations(set => {
          ...
          categories.forEach(category => {
            if (productCategoriesReduxState.has(category.id))
              set.add(category.id);
          });
        }),
      };
    ...
    return (
      <Formik
        ...
        validationSchema={yup.object({
          ...
          priceString: YUP_EDITING_FORM_PRICE_STRING('price').required(
            'price is a required field',
          ),
        })}
        onSubmit={(values, { setSubmitting }) => {
          submitForm(props.route.params.mealId, values)
            .finally(() => {
              setSubmitting(false);
            })
            .then(() => {
      ...
      >
        {formik => {
          ...
          return (
            ...
            >
              <FormikTextFieldView<MealEditOrCreationValues>
                topTitleText="Title"
