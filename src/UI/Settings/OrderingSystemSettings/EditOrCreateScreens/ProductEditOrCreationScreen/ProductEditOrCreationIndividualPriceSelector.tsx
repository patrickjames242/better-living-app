...
import {
  ...
  TextFieldTextInput,
} from '../../../../../helpers/Views/TextFieldView';
...
const ProductEditOrCreationIndividualPriceSelector = (() => {
  ...
  const ProductEditOrCreationIndividualPriceSelector = (
    props: ProductEditOrCreationIndividualPriceSelectorProps,
  ) => {
    const [
      ,
      {
        value: priceStringValue,
        error: priceStringError,
        touched: priceStringTouched,
      },
      { setValue: setPriceStringValue, setTouched: setPriceStringTouched },
    ] = useField<ProductEditOrCreateValues, 'priceString'>('priceString');
    const [
      ,
      { value: shouldBeSoldIndividuallyValue },
      { setValue: setShouldBeSoldIndividuallyValue },
    ] = useField<ProductEditOrCreateValues, 'shouldBeSoldIndividually'>(
      'shouldBeSoldIndividually',
    );

    return (
      <TextFieldViewContainer
        topTitleText="Individual Price"
        errorMessage={
          priceStringError && priceStringTouched ? priceStringError : undefined
        }
      >
        <SpacerView space={10} style={styles.contentHolder}>
          <TextFieldTextInput
            ...
            style={[
              styles.textField,
              { opacity: shouldBeSoldIndividuallyValue ? 1 : 0.4 },
            ]}
