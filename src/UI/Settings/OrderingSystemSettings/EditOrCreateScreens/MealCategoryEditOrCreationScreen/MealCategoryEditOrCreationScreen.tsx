...
import {
  deleteMealCategory,
  MealCategoriesRequestObj,
  createNewMealCategory,
  updateMealCategory,
} from '../../../../../api/orderingSystem/mealCategories/requests';
...
const MealCategoryEditOrCreationScreen = (() => {
  function submitForm(
    mealCategoryId: Optional<number>,
    values: MealCategoryEditOrCreateValues,
  ) {
    const displayName = (() => {
  ...
  const MealCategoryEditOrCreationScreen = (
    props: StackScreenProps<SettingsNavStackParams, 'MealCategoryEditOrCreate'>,
  ) => {
    const navBarTitle =
      props.route.params.mealCategoryId == null
      ...
      <Formik
        ...
        validationSchema={Yup.object({
          ...
          uniqueName: Yup.string()
            .trim()
            .required('unique name is a required field'),
        })}
        onSubmit={(values, { setSubmitting }) => {
          submitForm(mealCategoryId, values)
            .finally(() => {
              setSubmitting(false);
            })
            .then(() => {
      ...
      >
        {formik => {
          const shouldButtonsBeEnabled =
            formik.isSubmitting === false && isDeleting === false;
          ...
          return (
            <OrderingSystemEditingFormScreen
              ...
              deleteButtonProps={{
                ...
                isEnabled: shouldButtonsBeEnabled,
                onPress: () => {
                  if (typeof mealCategoryId !== 'number') {
                    return;
                  }
                  setIsDeleting(true);
            ...
            >
              <FormikTextFieldView<MealCategoryEditOrCreateValues>
                formikFieldName="uniqueName"
                topTitleText="Unique Name"
                textInputProps={DefaultKeyboardConfigs.name}
              />
              <FormikTextFieldView<MealCategoryEditOrCreateValues>
                formikFieldName="displayName"
                topTitleText="Display Name"
                textInputProps={DefaultKeyboardConfigs.name}
              />
              <OrderingSystemFormChildrenProductsSelector
                value={formik.values.productIds}
                onValueChanged={v => formik.setFieldValue('productIds', v)}
              />
            </OrderingSystemEditingFormScreen>
          );
        }}
