...
const OrderingSystemFormChildrenProductsSelector = (
  props: OrderingSystemFormChildrenProductsSelectorProps,
) => {
  const navigation =
    useNavigation<
      StackNavigationProp<SettingsNavStackParams, 'MealCategoryEditOrCreate'>
    >();
  ...
  const sortedProductsValue = useMemo(() => {
    return compactMap(props.value.toArray(), x =>
      allProductsReduxState.get(x),
    ).sort(caseInsensitiveStringSort(x => x.title));
  }, [allProductsReduxState, props.value]);

  return (
    <OrderingSystemFormChildrenSelector
