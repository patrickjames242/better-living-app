...
import {
  ...
  MenuEditOrCreateProductCategory,
  menuEditOrCreateProductCategorySchema,
} from './helpers';
...
import {
  ...
  createNewMenu,
  updateMenu,
} from '../../../../../api/orderingSystem/menus/requests';
...
const MenuEditOrCreationScreen = (() => {
  ...
  function parseTextFieldTimeString(timeString: string) {
    return moment.tz(
      timeString,
      timeStringTextFieldMomentFormat,
      NASSAU_TIME_ZONE,
    );
  }
  ...
  function textFieldTextFromAPITimeString(timeString: string) {
    return moment
      .tz(timeString, timeStringApiFormat, NASSAU_TIME_ZONE)
      .format(timeStringTextFieldMomentFormat);
  }
  ...
  const timeYupSchema = yup
    ...
    .matches(
      timeStringRegex,
      `This time text is formatted incorrectly. e.g. '3:30 pm'`,
    )
  ...
  async function validate(
    values: MenuEditOrCreationValues,
  ): Promise<Partial<Record<keyof MenuEditOrCreationValues, string>>> {
...

    {
      const productCategoriesError: string | undefined = (() => {
        ...
        while (currentNext.done !== true) {
          try {
            menuEditOrCreateProductCategorySchema.validateSync(
              currentNext.value,
            );
          } catch (error) {
        ...
        return titles.size < values.productCategories.size
          ? 'Two or more of the product categories have duplicate titles.'
          : undefined;
      })();
      productCategoriesError != undefined &&
        (errors.productCategories = productCategoriesError);
    }
  ...
  function submit(values: MenuEditOrCreationValues, menuId: Optional<number>) {
    ...
    return menuId == null
      ? createNewMenu(menuRequestObj)
      : updateMenu(menuId, menuRequestObj);
  }

  const MenuEditOrCreationScreen = (
    props: StackScreenProps<SettingsNavStackParams, 'MenuEditOrCreate'>,
  ) => {
    ...
    const initialValues: MenuEditOrCreationValues = useMemo(() => {
      ...
      return {
        ...
        startTime:
          mapOptional(menu?.startTime, x =>
            textFieldTextFromAPITimeString(x),
          ) ?? '',
        endTime:
          mapOptional(menu?.endTime, x => textFieldTextFromAPITimeString(x)) ??
          '',
        productCategories:
          mapOptional(menu?.categories.toArray(), x =>
            Map(
              x.map((category, index) => {
                const tuple: [number, MenuEditOrCreateProductCategory] = [
                  index,
                  {
                    customId: index,
                    title: category.title,
                    productIds: category.productIds.filter(x =>
                      reduxProductsState.has(x),
                    ),
                  },
                ];
                return tuple;
    ...
    return (
      <Formik<MenuEditOrCreationValues>
      ...
      >
        {formik => {
          const shouldButtonsBeEnabled =
            formik.isSubmitting === false && isDeleteLoading === false;
...
const FormChildren = (() => {
  ...
  const FormChildren = () => {
    return (
      <Spacer space={GenericEditingFormScreenConstants.childrenSpacing}>
