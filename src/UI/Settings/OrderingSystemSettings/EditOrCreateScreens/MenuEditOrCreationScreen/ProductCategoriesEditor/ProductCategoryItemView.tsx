...
const MenuEditOrCreationProductCategoryItemView = (() => {
  ...
  const MenuEditOrCreationProductCategoryItemView = ({
    ...
    onCategoryDeleted,
    errorMessage,
  }: MenuEditOrCreationProductCategoryItemViewProps) => {
    const [isExpanded, setIsExpanded] = useState(false);

    return (
      <TextFieldViewContainer
        errorMessage={
          isExpanded === false && errorMessage ? errorMessage : undefined
        }
      >
        <SpacerView
          style={[
            ...
            {
              backgroundColor:
                LayoutConstants.forms.innerContainer.backgroundColor,
            },
          ]}
          space={12}
        >
          {isExpanded ? (
            ...
            <CustomizedText style={styles.minimizedTitleText}>
              {category.title}
            </CustomizedText>
          )}
          <SpacerView
            style={[
              styles.sideButtonsHolder,
              {
          ...
          >
            <BouncySquareIconButton
              iconSources={{
                white: isExpanded
                  ? AssetImages.minimizeIcon
                  : AssetImages.zoomInIcon,
              }}
              iconSize={19}
...
const ExpandedView = (() => {
  ...
  const ExpandedView = (props: ExpandedViewProps) => {
    return (
      <SpacerView style={styles.root} space={15}>
        <TextFieldView
