...
const MenuEditOrCreationProductCategoriesEditor = () => {
  ...
  const errorMessageMap = useMemo(() => {
    const titleMap = Map<
      string,
      List<MenuEditOrCreateProductCategory>
    >().withMutations(set => {
      value.forEach(value => {
    ...
    const mapToReturn = Map<number, string>().withMutations(map => {
      value.map(x => {
        ...
        if (typeof validation === 'string') {
          ...
          map.set(
            x.customId,
            'Duplicate titles for product categories are not allowed.',
          );
        }
      });
    });
  ...
  return (
    <SpacerView space={GenericEditingFormScreenConstants.childrenSpacing}>
      {categoriesArray.length >= 1 && (
        <TextFieldViewContainer topTitleText="Product Categories">
          <Spacer
            space={LayoutConstants.floatingCellStyles.padding}
            // eslint-disable-next-line react/no-children-prop
            children={categoriesArray.map(oldCategory => {
              return (
                <MenuEditOrCreationProductCategoryItemView
                  ...
                  errorMessage={
                    errorMessageMap.get(oldCategory.customId) ?? null
                  }
                />
              );
            })}
