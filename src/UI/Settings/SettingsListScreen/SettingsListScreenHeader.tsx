...
const SettingsListScreenHeader = (() => {
  const styles = StyleSheet.create({
    ...
    initialsView: {
      borderRadius: 100000000,
      backgroundColor:
        CustomColors.themeGreen.withAdjustedOpacity(0.2).stringValue,
      width: 105,
      height: 105,
      alignItems: 'center',
  ...
  const getUppercaseFirstLetter = (s: string) =>
    s.substring(0, 1).toUpperCase();

  const SettingsListScreenHeader = (props: SettingsListScreenHeaderProps) => {
    return (
      <SpacerView style={styles.root} space={8}>
        <View style={styles.initialsViewHolder}>
          <View style={styles.initialsView}>
            <CustomizedText style={styles.initialsText}>
              {getUppercaseFirstLetter(props.userObject.firstName) +
                getUppercaseFirstLetter(props.userObject.lastName)}
            </CustomizedText>
          </View>
        </View>
        <Space space={20} />
        {[UserType.employee, UserType.manager].includes(
          props.userObject.userType,
        ) && (
        ...
        <CustomizedText style={styles.nameText}>
          {props.userObject.getFullName()}
        </CustomizedText>
        <CustomizedText style={styles.emailText}>
